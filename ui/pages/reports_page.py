...
class ReportsPage(QWidget):
    def __init__(self) -> None:
        super().__init__(); self.rows=[]; layout=QVBoxLayout(self); title=QLabel("Reports"); title.setObjectName("page_title"); layout.addWidget(title); controls=QHBoxLayout(); self.report=QComboBox(); self.report.addItems(["Sales Report","Inventory Report"]); self.period=QComboBox(); self.period.addItems(["Today","Yesterday","This Week","This Month"]); run=QPushButton("Run Report"); run.clicked.connect(self.run); export=QPushButton("Export"); export.clicked.connect(self.export); controls.addWidget(self.report); controls.addWidget(self.period); controls.addWidget(run); controls.addWidget(export); layout.addLayout(controls); self.table=QTableWidget(); layout.addWidget(self.table); self.run()
    def run(self) -> None:
        ...
        for r,row in enumerate(self.rows):
            for c,h in enumerate(headers): self.table.setItem(r,c,QTableWidgetItem(str(row[h])))
