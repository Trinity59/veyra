...
class POSPage(QWidget):
    def __init__(self) -> None:
        ...
        self.search.setPlaceholderText("Search product or code...")
        ...
        sale = QPushButton("Complete Sale")
        sale.clicked.connect(self.complete)
        actions.addWidget(clear)
        actions.addWidget(sale)
    ...
    def refresh(self) -> None:
        ...
        try:
            ...
            if term:
                query = query.where(
                    (Product.name.ilike(f"%{term}%")) | (Product.code.ilike(f"%{term}%"))
                )
            self.products.clear()
            for product in session.scalars(query):
                item = QListWidgetItem(
                    f"{product.name} | KSh {product.selling_price:,.2f} | Stock: {product.stock_quantity}"
                )
    ...
    def add_selected(self) -> None:
        ...
        quantity, accepted = QInputDialog.getInt(
            self, "Quantity", f"Available: {available}", 1, 1, available or 1
        )
        if not accepted:
            return
        if available <= 0 or self.cart.get(product_id, 0) + quantity > available:
            QMessageBox.warning(self, "POS", "Quantity exceeds available stock.")
            return
        ...
        try:
            ...
            for row, (product_id, quantity) in enumerate(self.cart.items()):
                ...
                values = (
                    product.name,
                    str(quantity),
                    f"KSh {product.selling_price:,.2f}",
                    f"KSh {product.selling_price * quantity:,.2f}",
                )
    ...
    def clear_cart(self) -> None:
        if self.cart:
            answer = QMessageBox.question(self, "Clear Cart", "Remove all items from the cart?")
    ...
    def complete(self) -> None:
        ...
        try:
            sale = complete_sale(
                session, [CartLine(product_id, quantity) for product_id, quantity in self.cart.items()]
            )
            session.commit()
            ...
            self.refresh_cart()
            self.refresh()
            QMessageBox.information(self, "Sale Completed", f"{invoice} — KSh {total:,.2f}")
        except Exception as exc:
            session.rollback()
            QMessageBox.critical(
                self, "Sale Error", f"Sale could not be completed. No stock was changed.\n{exc}"
            )
        finally:
            session.close()
